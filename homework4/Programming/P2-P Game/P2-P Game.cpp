...
ll evaluate_window(vector<int> &window, int piece){
	...
	ll player_cnt = array_count(window, piece), empty_cnt = array_count(window, EMPTY), opp_cnt = array_count(window, opp_piece); 
	if(player_cnt == 4)
		score += 500;
	else if(player_cnt == 3 && empty_cnt == 1)
		score += 70;
	else if(player_cnt == 2 && empty_cnt == 2)
		score += 8;

	if(opp_cnt == 3 && empty_cnt == 1)
		score -= 60;
...
ll score_position(vector<vector<int>> &board, int piece){
	...
	for(int i = 0;i < ROW;i++) center_array.push_back(board[i][COL/2]);
	...
	for(int r = 0;r < ROW;r++){
		...
		for(int i = 0;i < COL;i++) row_array.push_back(board[r][i]);
		for(int c = 0;c < COL-3;c++){
			vector<int> window;
	...
	for(int c = 0;c < COL;c++){
		...
		for(int i = 0;i < ROW;i++) col_array.push_back(board[i][c]);
		for(int r = 0;r < ROW-3;r++){
			vector<int> window;
	...
	for(int r = 0;r < ROW-3;r++){
		for(int c = 0;c <COL-3;c++){
			vector<int> window;
			for(int i = 0;i < WINDOW_LENGTH;i++) window.push_back(board[r+i][c+i]);
			score += evaluate_window(window, piece);
		}
	}
	...
	for(int r = 0;r < ROW-3;r++){
		for(int c = 0;c <COL-3;c++){
			vector<int> window;
			for(int i = 0;i < WINDOW_LENGTH;i++) window.push_back(board[r+3-i][c+i]);
			score += evaluate_window(window, piece);
		}
	}
...
int decide(int yp_move){
    if (yp_move == -1){
		...
		board.clear();
		board.resize(ROW);
		for(int i = 0;i < ROW;i++) board[i].resize(COL, 0);
		int col = 3;
		int row = get_next_open_row(board, col);
		...
		pair<int, ll> res = minmax(board, 3, NEG_INF, INF, true);
		int col = res.first;
